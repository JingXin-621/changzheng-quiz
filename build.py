# -*- coding: utf-8 -*-
"""生成长征问答挑战的 HTML 文件，确保 JS 中没有任何引号冲突"""
...
questions = [
    ...
    {"q":"长征途中被称为「党的历史上生死攸关的转折点」的会议是？","o":["八七会议","古田会议","遵义会议","瓦窑堡会议"],"a":2,"e":"1935年1月召开的遵义会议，纠正了左倾军事路线错误，确立了毛泽东在红军和党中央的领导地位，挽救了党、挽救了红军。","t":"重要会议"},
    ...
    {"q":"雪山上的军需处长为什么被冻死？","o":["迷路了","他把棉衣都发给了战士","没有足够棉衣","受伤行动不便"],"a":1,"e":"管被装的军需处长把棉衣全发给了战士，自己穿单衣冻死在雪山上，牺牲时仍保持坐姿。无名英雄的缩影。","t":"感人故事"},
...
# 使用 Python 的 json.dumps 来确保 JS 字符串完全合法
# json 会自动转义所有特殊字符
questions_json = json.dumps(questions, ensure_ascii=False)

css = '''\
:root{--r:#C41E3A;--rd:#8B0000;--rl:#E8485A;--g:#D4A853;--gl:#F0D68A;--bg:#FDF6F0;--c:#FFF;--t:#2C2C2C;--t2:#6B6B6B;--t3:#999;--b:#E8E0D5;--s:0 2px 12px rgba(0,0,0,.08);--sl:0 8px 32px rgba(0,0,0,.12);--R:16px;--r2:10px;--th:56px;--tab:64px;--sb:env(safe-area-inset-bottom,0px);--gn:#2E7D32}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif;background:var(--bg);color:var(--t);min-height:100vh;-webkit-font-smoothing:antialiased;-webkit-tap-highlight-color:transparent;user-select:none;overflow-x:hidden}
.header{position:sticky;top:0;z-index:100;background:linear-gradient(135deg,var(--rd),var(--r));color:#fff;height:var(--th);display:flex;align-items:center;justify-content:space-between;padding:0 16px;box-shadow:0 2px 8px rgba(139,0,0,.3)}
.header .logo{font-size:18px;font-weight:700;letter-spacing:1px;display:flex;align-items:center;gap:6px}
.streak{background:rgba(255,255,255,.2);border-radius:20px;padding:4px 12px;font-size:13px;display:flex;align-items:center;gap:4px}
.main{padding:16px 16px calc(var(--tab) + var(--sb) + 16px);max-width:720px;margin:0 auto}
.page{display:none}.page.on{display:block;animation:fade .3s}
@keyframes fade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
.card{background:var(--c);border-radius:var(--R);padding:18px;margin-bottom:14px;box-shadow:var(--s);border:1px solid var(--b)}
.ct{font-size:16px;font-weight:700;margin-bottom:14px;display:flex;align-items:center;gap:6px;color:var(--rd)}
.btn{height:46px;border:none;border-radius:var(--r2);font-size:15px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;padding:0 18px;font-family:inherit;transition:all .2s;white-space:nowrap}
.btn:active{transform:scale(.96)}
.btn1{background:linear-gradient(135deg,var(--r),var(--rl));color:#fff;box-shadow:0 4px 12px rgba(196,30,58,.3)}
.btn2{background:#FFF;color:var(--r);border:2px solid var(--r)}
.btnG{background:linear-gradient(135deg,var(--g),#B8860B);color:#fff;box-shadow:0 4px 12px rgba(184,134,11,.3)}
.btnL{height:54px;font-size:17px;width:100%}
.btnS{height:34px;font-size:12px;padding:0 12px;border-radius:16px}
.rank-box{text-align:center;padding:12px 0}
.rank-badge{width:90px;height:90px;margin:0 auto 10px;border-radius:50%;background:linear-gradient(135deg,var(--g),#B8860B);display:flex;align-items:center;justify-content:center;font-size:40px;color:#fff;box-shadow:0 4px 18px rgba(212,168,83,.5);position:relative}
.rank-badge::after{content:'';position:absolute;width:100px;height:100px;border-radius:50%;border:2px dashed var(--gl);opacity:.6;animation:rot 8s linear infinite}
@keyframes rot{from{transform:rotate(0)}to{transform:rotate(360deg)}}
.rank-name{font-size:22px;font-weight:800;color:var(--rd)}
.rank-sub{font-size:12px;color:var(--t3);margin-top:2px}
.exp-track{height:8px;background:#F0E6D8;border-radius:4px;overflow:hidden;margin-top:10px}
.exp-fill{height:100%;background:linear-gradient(90deg,var(--r),var(--g));border-radius:4px;transition:width .6s cubic-bezier(.4,0,.2,1)}
.exp-info{font-size:11px;color:var(--t3);text-align:right;margin-top:3px}
.stats{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px}
.stat{padding:12px 8px;text-align:center;background:linear-gradient(135deg,#FFF8F0,#FFF0E8);border-radius:var(--r2);border:1px solid #F0E0D0}
.sv{font-size:20px;font-weight:700;color:var(--rd)}
.sl{font-size:11px;color:var(--t3);margin-top:1px}
.quiz-num{font-size:12px;color:var(--r);font-weight:600;margin-bottom:6px}
.quiz-dots{display:flex;gap:5px;justify-content:center;margin-bottom:14px}
.qd{width:8px;height:8px;border-radius:50%;background:#E0D5C5;transition:all .3s}
.qd.done{background:var(--gn)}.qd.wrong{background:var(--rl)}.qd.cur{background:var(--g);transform:scale(1.4)}
.quiz-timer{height:4px;background:#F0E6D8;border-radius:2px;margin-bottom:14px;overflow:hidden}
.quiz-timer-fill{height:100%;background:var(--gn);border-radius:2px;transition:width .1s linear}
.quiz-timer-fill.warn{background:var(--rl)}
.quizQ{font-size:18px;font-weight:700;line-height:1.6;margin-bottom:16px;color:var(--t)}
.quizOpts{display:flex;flex-direction:column;gap:8px}
.qOpt{width:100%;padding:14px 16px;text-align:left;border:2px solid var(--b);border-radius:var(--r2);background:#FFF;font-size:14px;font-weight:500;cursor:pointer;display:flex;align-items:center;gap:10px;font-family:inherit;color:var(--t);transition:all .2s}
.qOpt:hover{border-color:var(--r);background:#FFF8F8}
.qOpt:active{transform:scale(.98)}
.qOpt .ol{width:28px;height:28px;border-radius:50%;background:#F5F5F5;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;flex-shrink:0;color:var(--t2)}
.qOpt.ok{border-color:var(--gn)!important;background:#F1F8E9!important;animation:pulse .5s}
.qOpt.ok .ol{background:var(--gn);color:#fff}
.qOpt.no{border-color:var(--rl)!important;background:#FFF0F0!important;animation:shake .4s}
.qOpt.no .ol{background:var(--rl);color:#fff}
@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.02)}}
@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-6px)}50%{transform:translateX(6px)}75%{transform:translateX(-4px)}}
.qExp{margin-top:14px;padding:14px;border-radius:var(--r2);background:#FFF8E1;border:1px solid #FFE082;font-size:13px;line-height:1.7;color:#5D4037;display:none}
.qExp.s{display:block;animation:fade .3s}
.qExp .et{font-weight:700;color:#E65100;margin-bottom:2px}
.qRes{text-align:center;padding:16px 0}
.rIcon{font-size:60px;margin-bottom:8px}
.rScore{font-size:44px;font-weight:900;color:var(--r)}
.rLabel{font-size:13px;color:var(--t3);margin:6px 0 10px}
.rDetail{font-size:14px;line-height:1.8;color:var(--t2)}
.ach-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
.ach{padding:10px;border-radius:var(--r2);background:#F9F9F9;text-align:center;border:1px solid var(--b);transition:all .3s}
.ach.got{background:linear-gradient(135deg,#FFF8E1,#FFF3CD);border-color:var(--g)}
.ach .ai{font-size:26px}.ach .an{font-size:12px;font-weight:600;margin-top:2px}.ach .ad{font-size:10px;color:var(--t3)}
.ach.lock{opacity:.4;filter:grayscale(1)}
.rank-tabs{display:flex;margin-bottom:14px;background:var(--c);border-radius:var(--r2);overflow:hidden;box-shadow:var(--s);border:1px solid var(--b)}
.rtab{flex:1;padding:12px;text-align:center;font-size:13px;font-weight:600;cursor:pointer;color:var(--t2);border:none;background:transparent;font-family:inherit;transition:all .2s}
.rtab.on{background:var(--r);color:#fff}
.rlist{display:flex;flex-direction:column;gap:6px}
.ritem{display:flex;align-items:center;gap:10px;background:var(--c);border-radius:var(--r2);padding:12px 14px;box-shadow:var(--s);border:1px solid var(--b);transition:all .2s}
.ritem.me{border:2px solid var(--r);background:linear-gradient(135deg,#FFF5F5,#FFF)}
.rav{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,var(--r),var(--rl));color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:15px;flex-shrink:0}
.rinfo{flex:1;min-width:0}.rn{font-size:14px;font-weight:600}.rsub{font-size:11px;color:var(--t3)}.rs{font-size:15px;font-weight:700;color:var(--r);flex-shrink:0}
.klist{display:flex;flex-direction:column;gap:8px}
.kitem{background:var(--c);border-radius:var(--r2);padding:14px;box-shadow:var(--s);border:1px solid var(--b);cursor:pointer;transition:all .2s}
.kitem:hover{border-color:var(--r)}
.kitem .kq{font-size:14px;font-weight:600;margin-bottom:4px}
.kitem .ka{font-size:13px;color:var(--gn)}
.kitem .ke{font-size:12px;color:var(--t2);margin-top:4px;line-height:1.6}
.kitem .kt{display:inline-block;background:#FFF0E0;color:#B8860B;padding:2px 8px;border-radius:10px;font-size:10px;margin-top:6px}
.toast{position:fixed;top:80px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#2C2C2C,#444);color:#FFD700;padding:12px 22px;border-radius:28px;font-weight:700;font-size:14px;z-index:300;box-shadow:0 8px 24px rgba(0,0,0,.3);animation:tin .4s ease,tout .4s ease 2.5s forwards;white-space:nowrap;pointer-events:none}
@keyframes tin{from{opacity:0;transform:translateX(-50%) translateY(-20px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}
@keyframes tout{from{opacity:1}to{opacity:0;transform:translateX(-50%) translateY(-20px)}}
.tab-bar{position:fixed;bottom:0;left:0;right:0;height:calc(var(--tab) + var(--sb));padding-bottom:var(--sb);background:#FFF;display:flex;border-top:1px solid var(--b);z-index:100;box-shadow:0 -2px 8px rgba(0,0,0,.04)}
.tab-i{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;cursor:pointer;color:var(--t3);border:none;background:none;font-family:inherit;font-size:10px;transition:color .2s}
.tab-i .ti{font-size:20px}.tab-i.on{color:var(--r);font-weight:600}
@media(min-width:768px){.header{position:fixed;top:0;left:0;right:0}.tab-bar{position:fixed;top:var(--th);left:0;bottom:auto;width:180px;height:calc(100vh - var(--th));flex-direction:column;padding:8px 0;border-right:1px solid var(--b);border-top:none;box-shadow:none}.tab-i{flex:none;flex-direction:row;padding:12px 18px;gap:10px;font-size:14px;justify-content:flex-start;border-radius:0 24px 24px 0;margin-right:10px}.main{margin-left:180px;margin-top:var(--th);padding:20px 28px 28px;max-width:calc(100% - 180px)}.ach-grid{grid-template-columns:repeat(3,1fr)}.rlist{display:grid;grid-template-columns:1fr 1fr;gap:8px}}
'''
...
function shareText(){{var rk=gRk(D.s),pct=D.ta>0?Math.round(D.tc/D.ta*100):0;var t="🚩 长征红色问答挑战\\n\\n⭐ 当前军衔："+rk.n+" | 总积分："+D.s+"\\n✅ 累计答对："+D.tc+"题 | 正确率："+pct+"%\\n🔥 连续打卡："+D.st+"天\\n\\n📖 学习长征历史，传承红色基因！\\n👉 快来加入挑战吧！";if(navigator.share){{navigator.share({{title:"长征红色问答挑战",text:t}}).catch(function(){{}})}}else{{copyText(t);alert("已复制到剪贴板！可以粘贴分享到微信/朋友圈～")}}}}
...
# Also validate by extracting JS and running node --check
import subprocess
import tempfile
import os
...
tmp = os.path.join(tempfile.gettempdir(), 'validate_js.js')
